	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (op != null && op.equals("delete")) {
			...
			if (InputsValidation.isValid("numeric", _id)) {
				...
				int _deletResult = Employee.deleteEmployee(id);
				if(_deletResult==-1) {
		...
		return;
			...
			getServletContext().getRequestDispatcher("/WEB-INF/dashboard/employee.jsp").forward(request, response);
			return;
		}
		...
		if (op != null) {
			...
			case "add":
				...
				if (InputsValidation.isValid("email", email) && InputsValidation.isValid("alphanumeric", password)
						&& InputsValidation.isValid("alphabet", firstName)
						&& InputsValidation.isValid("alphabet", lastName)
						&& InputsValidation.isValid("date", dateOfBirth) && InputsValidation.isValid("gender", gender)
						&& InputsValidation.isValid("alphNumSpace", street)
						&& InputsValidation.isValid("alphabet", city) && InputsValidation.isValid("alphabet", state)
						&& InputsValidation.isValid("numeric", zipCode) && InputsValidation.isValid("numeric", phone)) {
					Date _dateOfBirth;
					try {
					...
					int _addResult = Employee.addEmployee(email, password, firstName, lastName, _dateOfBirth, gender,
							phone, street, city, state, zipCode);
					if (_addResult == -1){
						ses.setAttribute("msg", "Employee add ");
						ses.setAttribute("alartType", "Success");
					}
					else if(_addResult==1062) {
						ses.setAttribute("msg", "Employee is exist");
						ses.setAttribute("alartType", "Warning");
					}
					else  {
			...
			case "edit":
				...
				if (InputsValidation.isValid("numeric", id) && InputsValidation.isValid("email", email)
						&& InputsValidation.isValid("alphabet", firstName)
						&& InputsValidation.isValid("alphabet", lastName)
						&& InputsValidation.isValid("date", dateOfBirth) && InputsValidation.isValid("gender", gender)
						&& InputsValidation.isValid("alphNumSpace", street)
					...
					int _editResult = Employee.editEmployee(_id, email, password, firstName, lastName, _dateOfBirth,
							gender, phone, street, city, state, zipCode);
					if (_editResult == -1){
						ses.setAttribute("msg", "Employee update ");
						ses.setAttribute("alartType", "Success");
					}
